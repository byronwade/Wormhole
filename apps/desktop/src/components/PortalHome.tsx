  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 80% 50% at 15% -5%, rgba(124,58,237,0.28), transparent 55%), radial-gradient(ellipse 55% 40% at 95% 90%, rgba(20,184,166,0.08), transparent 50%), #0F0F0F",
        }}
      />
      <div className="portal-grain pointer-events-none absolute inset-0 opacity-60" aria-hidden />

      <div className="relative z-10 flex-1 overflow-y-auto px-6 py-9 md:px-12">
        <header className="mb-10 max-w-xl">
          <p className="font-mono-brand mb-4 text-[11px] uppercase tracking-[0.38em] text-[#A78BFA]">
            Wormhole · {deviceName}
          </p>
          <h1 className="font-display mb-3 text-4xl font-semibold tracking-tight text-[#FAFAFA] sm:text-5xl">
            Portal
          </h1>
        ...
        {liveMount ? (
          <div className="mb-10 max-w-lg motion-peer-in">
            <MountStatusStrip
          ...
          <div className="mb-10 flex flex-wrap gap-3">
            <Button
              onClick={onShare}
              className="min-h-12 px-7 bg-[#7C3AED] text-white shadow-[0_0_32px_rgba(124,58,237,0.25)] hover:bg-[#6D28D9]"
            >
            ...
            <Button
              onClick={onConnect}
              variant="outline"
              className="min-h-12 border-zinc-700 px-7 text-zinc-100 hover:border-[#7C3AED]/50"
            >
        ...
        {remotePeers.length > 0 && (
          <section className="mb-12 max-w-xl" aria-labelledby="nearby-heading">
            <h2
              id="nearby-heading"
              className="mb-4 flex items-center gap-2 text-xs font-medium uppercase tracking-[0.2em] text-zinc-500"
            >
            ...
            <ul className="space-y-2">
              {remotePeers.map((peer, i) => (
                <li
                  key={peer.id}
                  className="motion-peer-in flex items-center gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.03] px-4 py-3.5 backdrop-blur-sm"
        ...
        <section className="max-w-xl" aria-labelledby="sessions-heading">
          ...
          {empty ? (
            <div className="rounded-2xl border border-dashed border-white/[0.08] px-6 py-14 text-center">
              <p className="mb-1 text-zinc-300">No active tunnels</p>
              ...
              <div className="mt-6 flex flex-wrap justify-center gap-2">
                ...
                <Button onClick={onConnect} size="sm" variant="outline" className="min-h-10 border-zinc-700">
                  Enter a code
                </Button>
              </div>
            </div>
        ...
        {liveMount && (
          <div className="mt-8 flex flex-wrap gap-2">
            <Button
              ...
              variant="outline"
              size="sm"
              className="min-h-10 border-zinc-800 text-zinc-400"
            >
              Share another…
            </Button>
            <Button
...
function SessionRow({
  ...
  return (
    <li
      className={cn(
        "motion-peer-in group relative flex items-stretch overflow-hidden rounded-2xl border",
        isLive
          ? "border-[#7C3AED]/30 bg-gradient-to-r from-[#7C3AED]/10 via-transparent to-transparent"
    ...
    >
      <div
        className={cn(
          ...
          session.status === "connecting" && "bg-amber-400 motion-status-pulse",
          session.status === "error" && "bg-red-400",
          (session.status === "idle" || session.status === "expired") && "bg-zinc-700",
        )}
        aria-hidden
      />
      <div className="flex min-w-0 flex-1 items-center gap-3 px-4 py-3.5">
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
            ...
            <span className="text-[10px] uppercase tracking-[0.16em] text-zinc-500">{modeLabel}</span>
            {session.speedLabel && (
