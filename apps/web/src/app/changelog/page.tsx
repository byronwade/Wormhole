...
export default function ChangelogPage() {
  ...
  function parseMarkdown(text: string) {
    ...
    return text
      ...
      .replace(/^- (.+)$/gm, '<li class="text-muted-foreground ml-4">$1</li>')
      .replace(/`([^`]+)`/g, '<code class="px-1 py-0.5 bg-muted rounded text-wormhole-hunter-light text-sm">$1</code>')
      ...
      <section className="py-16 px-6 border-b border-border">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-4 mt-6">
            <Button variant="outline" className="border-border text-muted-foreground hover:bg-muted" render={<a href={`https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}/releases`} target="_blank" rel="noopener noreferrer" />}>
              ...
              {releases.map((release, index) => (
                <article key={release.id} className="relative">
                  ...
                  {index < releases.length - 1 && (
                    <div className="absolute left-[11px] top-12 bottom-0 w-px bg-muted" />
                  )}
                  ...
                  <div className="flex gap-6">
                    ...
                    <div className="flex-1 pb-12">
                      ...
                      {release.body ? (
                        ...
                        <p className="text-muted-foreground italic">No release notes provided.</p>
                      )}
                    </div>
                  </div>
                </article>
      ...
      <section className="py-16 px-6 border-t border-border bg-card/30">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            ...
            <Button size="lg" variant="outline" className="border-border text-muted-foreground hover:bg-muted" render={<Link href="/#download" />}>
              Download Latest
